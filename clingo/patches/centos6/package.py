...
class ClingoBootstrap(Clingo):
    ...
    @run_after('cmake')
    def capsulethunk(self):
        # Copy capsulethunk.h into place
        pkg_dir = os.path.dirname(__file__)
        target_dir = os.path.join(self.stage.source_path, 'libpyclingo')	

        for filename in ('capsulethunk.h', 'pyclingo.cc'):
            source_path = os.path.join(pkg_dir, filename)
