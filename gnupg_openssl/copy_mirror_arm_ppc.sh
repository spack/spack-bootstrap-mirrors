...
id=$(docker create --platform linux/arm64 ghcr.io/alalazo/gnupg_openssl_manylinux2014:latest)
mkdir arm64
docker cp $id:/home/spack/binary-mirror arm64/binary-mirror
docker rm -v $id

id=$(docker create --platform linux/ppc64le ghcr.io/alalazo/gnupg_openssl_manylinux2014:latest)
mkdir ppc64le
